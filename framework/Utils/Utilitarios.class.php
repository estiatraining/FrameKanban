...
class Utilitarios extends Sessoes {
    ...
    public function getBean($request, $bean) {
        $this->request = $request;
        $this->bean = $bean;
        $return = "";
        foreach ($this->request as $campos => $valores) {
            foreach ($this->bean as $atributos => $setValues) {
                if ($campos == $atributos) {
    ...
    //metodo i18n que retorna o termo no idioma recuperado via dados do browser do usuario
    // ou via cookie que foi setado pelo usuario ao escolher outro idioma
    //@param $texto é o termo que sera retornado
    public function i18n($texto) {
        $locale = "en";
        $contexto = "";
        $retorno = "";
        ...
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . $contexto . '/framework/I18N/messages_' . $locale . '.ini')) {
            $dados = parse_ini_file($_SERVER['DOCUMENT_ROOT'] . '/' . $contexto . '/framework/I18N/messages_' . $locale . '.ini');
            $retorno = $dados[$texto];
        }
        return $retorno;
    }
    ...
    public function getContexto() {
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/kanban/framework/sysConf/contexto.ini')) {
            $dados = parse_ini_file($_SERVER['DOCUMENT_ROOT'] . '/kanban/framework/sysConf/contexto.ini');
    ...
    public static function findString($_text, $_string) {
    ...
    public function populaObject($form, $bean) {
        foreach ($form as $campos => $valores) {
            echo "<script>alert('" . $campos . " - " . $atributos . "');</script>";
            foreach ($bean as $atributos => $valoresAtr) {
                echo "<script>alert('" . $campos . " - " . $atributos . "');</script>";
                if ($campos == $atributos) {
                    $bean->$atributos = $valores;
                }
            }
        }
        return $bean;
